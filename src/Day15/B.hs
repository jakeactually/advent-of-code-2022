...
-- based on this solution https://www.reddit.com/r/adventofcode/comments/zmcn64/comment/j3603ze/
-- which happens to be similar to what I made for part A
...
calculateLineRange :: ((Int, Int), (Int, Int)) -> Int ->Maybe (Int, Int)
calculateLineRange ((sx, sy), (bx, by)) customY
    | lineLength < 0 = Nothing
    | otherwise = Just (lineStart, lineEnd)
    where
        distToBeacon = manhattan (sx, sy) (bx, by)
        distToAnchor = manhattan (sx, sy) (sx, customY)
...
sensorsToLimits :: [((Int, Int), (Int, Int))] -> Int -> [Limit]
sensorsToLimits sensors customY = sortOn limitToInt $ concatMap tupleToLimit results
    where
        results = mapMaybe (`calculateLineRange` customY) sensors
